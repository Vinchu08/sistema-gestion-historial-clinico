...
export default async function LeftSidebar({
  ...
  const patients = await fetchFilteredPatients(patientQuery);

  return (
    <div className="w-2/12 bg-gray-200 p-4 overflow-y-auto h-full">
      <h3 className="text-lg font-semibold mb-4">Pacientes</h3>

      <SearchPatient placeholder="Buscar pacientes..." queryParam="patientQuery" />
      ...
      <ul>
        {patients?.map((patient) => (
          <li
            ...
            className="mb-2 p-2 bg-white rounded-lg shadow cursor-pointer hover:bg-gray-300 transition flex items-center space-x-3"
          >
            <img src={patient.image} alt={patient.fullname} className="w-8 h-8 rounded-full" />
